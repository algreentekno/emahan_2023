...
// getting total number records without any search
    ...
    $sql = "SELECT * from kumuh_luas a inner join kecamatan d on a.kode_kec=d.kode_kec inner join kelurahan e on a.kode_kel=e.kode_kel inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where $filter and a.klasifikasi_kumuh='2' order by a.Id_kumuh_luas";
...
if( !empty($requestData['search']['value']) ) {
    // if there is a search parameter
        ...
        $sql.=" where ($filter and a.klasifikasi_kumuh='2' AND a.no_sk LIKE '%".$requestData['search']['value']."%')";
        $sql.=" OR ($filter and a.klasifikasi_kumuh='2' AND a.lokasi LIKE '%".$requestData['search']['value']."%')";
    ...
    $query=mysqli_query($koneksi, $sql) or die(""); // again run query with limit
        ...
        $sql = "SELECT * ";
        $sql.=" from kumuh_luas a inner join kecamatan d on a.kode_kec=d.kode_kec inner join kelurahan e on a.kode_kel=e.kode_kel inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where $filter and a.klasifikasi_kumuh='2'";
