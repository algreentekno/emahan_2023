...
if($Lvl=='0' OR $Lvl=='1'){
$sql = "SELECT * from air_limbah_akses a  inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where a.kode_kota='$kodekota' and a.tahun_data='$tahundata' order by a.Id_limbahakses";
}else{
    $sql = "SELECT * from air_limbah_akses a  inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where a.kode_kota='$kodekota' and a.tahun_data='$tahundata' order by a.Id_limbahakses";
...
if( !empty($requestData['search']['value']) ) {
    ...
    if($Lvl=='0' OR $Lvl=='1'){
    $sql = "SELECT * ";
    $sql.=" FROM air_limbah_akses a inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where a.kode_kota='$kodekota' and a.tahun_data='$tahundata'";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    ...
    if($Lvl=='3' or $Lvl=='0'){
        if($row['validasi']=='0'){
            ...
            <a href="#" data-bs-toggle="modal" data-bs-target="#modaledit" data-id="'.$row['Id_limbahakses'].'" class="btn btn-sm btn-round btn-icon btn-success"><em class="icon ni ni-edit"></em></a>
            <a href="#" data-bs-toggle="modal" data-bs-target="#modallihat" data-id="'.$row['Id_limbahakses'].'" class="btn btn-sm btn-round btn-icon btn-info"><em class="icon ni ni-eye"></em></a>
