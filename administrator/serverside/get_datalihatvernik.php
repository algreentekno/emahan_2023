...
$sql = "SELECT a.tahun_verifikasi,a.ket_verifikasi,a.sts_verifikasi,a.Id_rtlh,a.tahun_dibantu,a.sumber_dana,a.sts_penanganan,a.tahun_verifikasi,a.sts_verifikasi,d.nama_kota,c.nama_kecamatan,b.nama_kelurahan,a.no_ktp,a.nama_kk,a.luas_rumah,a.alamat_rumah,a.sts_kepemilikan_rumah,a.jenis_kawasan FROM  rtlh a INNER JOIN kelurahan b on a.kode_kel=b.kode_kel INNER JOIN kecamatan c on b.kode_kec=c.kode_kec INNER JOIN kota d on c.kode_kota=d.kode_kota where a.sts_verifikasi='Y' and $sqlq  order by a.Id_rtlh";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql.=" where (a.sts_verifikasi='Y' and $sqlq  and a.nama_kk LIKE '%".$requestData['search']['value']."%')";// $requestData['search']['value'] contains search parameter
    $sql.=" OR (a.sts_verifikasi='Y' and $sqlq  and a.no_ktp LIKE '%".$requestData['search']['value']."%')";
    ...
    $sql="SELECT a.tahun_verifikasi,a.ket_verifikasi,a.sts_verifikasi,a.Id_rtlh,a.tahun_dibantu,a.sumber_dana,a.sts_penanganan,a.tahun_verifikasi,a.sts_verifikasi,d.nama_kota,c.nama_kecamatan,b.nama_kelurahan,a.no_ktp,a.nama_kk,a.luas_rumah,a.alamat_rumah,a.sts_kepemilikan_rumah,a.jenis_kawasan FROM  rtlh a INNER JOIN kelurahan b on a.kode_kel=b.kode_kel INNER JOIN kecamatan c on b.kode_kec=c.kode_kec INNER JOIN kota d on c.kode_kota=d.kode_kota where a.sts_verifikasi='Y' and $sqlq ";
