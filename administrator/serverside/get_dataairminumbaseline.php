...
$sql = "SELECT * from air_minum_capaian a inner join kecamatan d on a.kode_kec=d.kode_kec inner join kelurahan e on a.kode_kel=e.kode_kel inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where a.jenis='Baseline' order by a.Id_air_capaian";
...
if( !empty($requestData['search']['value']) ) {
    ...
    $sql.=" where (a.jenis='Baseline' AND a.tahun LIKE '%".$requestData['search']['value']."%')"; 
    $sql.=" OR (a.jenis='Baseline' AND d.nama_kecamatan LIKE '%".$requestData['search']['value']."%')";     // $requestData['search']['value'] contains search parameter
    $sql.=" OR (a.jenis='Baseline' AND e.nama_kelurahan LIKE '%".$requestData['search']['value']."%')";     // $requestData['search']['value'] contains search parameter
    $sql.=" OR (a.jenis='Baseline' AND b.nama_kota LIKE '%".$requestData['search']['value']."%')";     // $requestData['search']['value'] contains search parameter
    ...
    $sql = "SELECT * ";
    $sql.=" from air_minum_capaian a inner join kecamatan d on a.kode_kec=d.kode_kec inner join kelurahan e on a.kode_kel=e.kode_kel inner join kota b on a.kode_kota=b.kode_kota inner join provinsi c on a.kode_prov=c.kode_prov where a.jenis='Baseline'";
